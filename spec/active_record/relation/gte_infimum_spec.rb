...
RSpec.describe ActiveRecord::QueryMethods::WhereChain do
  ...
  describe '#gte_infimum' do
    let (:int4range) { (1..10) }
    let (:array) { [1, 2, 3] }

    let (:range_operator) { Post.where.gte_infimum(int4range: int4range).exists? }
    let (:array_operator) { Post.where.gte_infimum(array: array).exists? }
